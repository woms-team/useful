...
def get_current_user(db: Session = Depends(get_db), token: str = Security(reusable_oauth2)):
    ...
    user = service.user.get(db, id=token_data.user_id)
    if not user:
...
def registration_user(new_user: schemas.UserCreateInRegistration, db: Session):
    ...
    if service.user.get_by_username_email(db, username=new_user.username, email=new_user.email):
        return True
    else:
        user = service.user.create(db, obj_in=new_user)
...
def verify_registration_user(uuid: VerificationInDB, db: Session):
    ...
    if verify:
        user = service.user.get(db, verify.user_id)
        service.user.update(db, db_obj=user, obj_in=schemas.UserUpdate(**{"is_active": "true"}))
